package com.rindus.task.restconsumer.model;

public class DataProducerConstants {

	public final static String CONCURENT_PROCESS_DATA_ERROR = "Error during producing data ";
	public final static String RESOURCE_DELETED = "Resource deleted, id: ";
	public final static String REMOTE_API_SERVER_ERROR= "Remote API server error: ";
	public final static String RESOURCE_AVAILABLE= "Resource found by id: ";
	public final static String RESOURCE_LIST_AVAILABLE= "Resource list is available, size : ";	
	public final static String UPDATE_RESOURCE_BY_ID= "Update resource, id: ";
	
	public final static String RESOURCE_NOT_FOUND= "Resource not found";
}
